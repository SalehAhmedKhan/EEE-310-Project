...
%% noise graph plotting

noisedB = [20 30 40 50 60 70 80 90];

noisy_snr = zeros(size(noisedB));
...
for i = 1:length(noisedB)
    ...
    sig = @(t) sin(2*pi*72*t) + sin(2*pi*275*t) + sin(2*pi*420*t);
    %sig = @(t)sin(2*pi*25*t) + 0.5*sin(2*pi*345*t) + sin(2*pi*120*t) + 2*sin(2*pi*63*t);
    %sig = @(t) sin(2*pi*100*t);
    %sig = @(t) sinc(53*t - 15);
    %sig = @(t) sinc(75*t - 25).*sinc(75*t - 25);
    %sig = @(t) rectangularPulse(-1,0.5,t);
    ...
    noise_w = sqrt(samp_sig*samp_sig'*10^(-noisedB(i)/10))*randn(length(sub_x),1);
    %sub_x=sub_x;
    sub_x=sub_x + noise_w;
...
plot(noisedB,noisy_snr,'b*-')
xlabel('Input SNR')
ylabel('Output SNR')
title('Reconstructed SNR vs Added Noise SNR')
